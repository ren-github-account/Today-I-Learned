...
そして、`inline-block要素`やi`nline-table要素`は**独立した不透明なボックスとして、インラインフォーマットコンテキストに参加する。**
...
つまり、この場合、div要素の生成するブロックコンテナボックスの中に、ブロックレベルボックスとインラインボックスが混在していることになる。

この場合、コンテンツを扱いやすくするためにテキストの周囲に匿名ブロックボックスがあるという風に強制して、  
ブロックコンテナボックスの中に、ブロックレベルボックスのみがあるようにする仕様がある。
