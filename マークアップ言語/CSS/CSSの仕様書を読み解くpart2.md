...
displayプロパティの値が、inline-blockやinline-tableの`inline-block要素`やi`nline-table要素`は**別名アトミックインラインレベルボックスと呼ばれる。**

そして、`inline-block要素`やi`nline-table要素`は独立した不透明なボックスとして、インラインフォーマットコンテキストに参加する。
