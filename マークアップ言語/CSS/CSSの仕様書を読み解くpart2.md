...
2つとも、**フォーマット(配置)を定義しやすくするためにつくられた仮のボックス**である点は同じ。



テキストはインラインコンテンツであり、各要素が生成するインラインボックスの中に存在している。

インラインボックスは、インライン要素だけではなく、p要素などのブロックレベル要素でも生成される。  
そのため、例えば、以下のような場合、テキストの「こんにちわ」は、p要素によって生成されるインラインボックスの中にあることになる。

```
<p>こんにちわ</p>
```

そして、テキストの間にemやspanなどがある場合に、匿名インラインボックスができる。  
例を挙げると以下になる。
