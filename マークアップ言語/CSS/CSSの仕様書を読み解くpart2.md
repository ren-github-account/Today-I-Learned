...
まとめると、p要素が生成したブロックボックスの中に、インラインボックスがあり、そのインラインボックスの中にインラインコンテンツであるテキストの「こんにちわ」が存在している。

そして、テキストの間にemやspanなどがある場合、そのほかのテキストが存在するインラインボックスは**匿名インラインボックスと呼ばれる。**
例を挙げると以下になる。
