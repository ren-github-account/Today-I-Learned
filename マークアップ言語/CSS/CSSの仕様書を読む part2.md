...
そして、その要素は`display:inline`の時インラインボックスを生成する。

インラインボックスは、displayプロパティの値が`inline, inline-table, inline-block`のどれかであり、そのcontentstがインラインフォーマットコンテキストに参加しているボックスのこと。
