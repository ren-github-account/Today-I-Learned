...
上記に挙げた仕様書の具体例では、divの生成するブロックコンテナボックスの中に、インラインレベルのコンテンツとブロックレベルのコンテンツが混在している場合が示されているが、以下のように、**div要素の中に「p要素」のみがある場合もブロックボックスとなる**。
...
このことから、シンプルに**div要素などのブロックレベル要素の中に別の要素やコンテンツが1個でも含まれていたら、ブロックボックスとなる**と考えてok。
...
replaced elementが「内容が CSS 書式設定モデルの範囲外にある要素。(具体的には、画像を入れる時に使うimg要素など)」のことなので、**non-replaced elementはCSSの適用範囲内にある通常の要素ということになる。**
...
||意味|
|-|-|
|インラインボックス|displayプロパティの値が`inline, inline-block, inline-table`の時、その要素はインラインレベル要素となり、インラインレベルボックスを生成する。インラインレベルボックスは、**そのcontentsがそのボックスを含んでいる状態で**インラインフォーマットコンテキストに参加している時に限り、別名 **「インラインボックス」** と呼ばれる。|
