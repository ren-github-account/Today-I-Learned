...
* [アトミックインラインレベルボックスとは](#アトミックインラインレベルボックスとは)
...
|ブロックコンテナボックス|なかに要素が入るコンテナ **(容器)**　のこと。例えば、ブロックレベルボックスの中にp要素などが含まれるような時、そのブロックレベルボックスはブロックコンテナボックスとなる。**(※すべてのブロックコンテナボックスがブロックレベルボックスであるわけではない。つまりブロックレベルボックスではないブロックコンテナボックスもあるということ。)**|
...
**インラインボックスとは、インラインレベルボックスが、インラインフォーマットコンテキストに参加している時の呼び名のこと。**

replaced elementが「内容が CSS 書式設定モデルの範囲外にある要素。」のことなので、non-replaced elementはCSSの適用範囲内にある通常の要素ということになる。  
そして、displayプロパティの値が`inline`であるnon-replaced elementはインラインボックスを生成する。

インラインレベルボックスが、インラインフォーマットコンテキストに参加している時には別名「インラインボックス」と呼ばれると先ほど書いたけど、インラインレベルボックスであってもインラインボックスではないボックスもありその場合は注意が必要である。
...
||意味|
|-|-|
|インラインボックス|displayプロパティの値が`inline, inline-block, inline-table`の時、その要素はインラインレベル要素となり、インラインレベルボックスを生成する。インラインレベルボックスは、**そのcontentsがそのボックスを含んでいる状態で**インラインフォーマットコンテキストに参加している時に限り別名 **「インラインボックス」** と呼ばれる。|
|アトミックインラインレベルボックス|インラインレベルボックスであっても「そのcontentsがそのボックスを含んでいる状態」ではなく、**独立した不透明なボックスとして**インラインフォーマットコンテキストに参加している場合は、アトミックインラインレベルボックスと呼ばれる。例:`display:inline-block`の時など。|
...
displayプロパティの値が、inline-blockやinline-tableの`inline-block要素`や`inline-table要素`は**別名アトミックインラインレベルボックスと呼ばれる。**

別名アトミックインラインレベルボックスと呼ばれるのは、`inline-block要素`や`inline-table要素`が**独立した不透明なボックスとして、インラインフォーマットコンテキストに参加することが理由だ。**

まとめると、**アトミックインラインレベルボックスとは、独立した不透明なボックスとして、インラインフォーマットコンテキストに参加する要素のこと**。

**具体的には、`inline-block要素`や`inline-table要素`がある**
