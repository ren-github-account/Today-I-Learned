...
>ボックスにテキストまたは他のボックスが含まれているかどうか、などのいくつかの要因によって決まります。 ボックスはテーブルなどです。ボックスの幅と高さについては、ビジュアル フォーマット モデルの詳細に関する章で説明します。
>ボックスのコンテンツ、パディング、境界領域の背景スタイルは、生成要素の 'background' プロパティによって指定されます。 余白の背景は常に透明です。
