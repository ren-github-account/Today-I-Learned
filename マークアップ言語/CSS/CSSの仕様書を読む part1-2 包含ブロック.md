...
この「CSSの仕様書を読む」シリーズは現在part3まで書いているが、仕様書を読み解いていくにあたってどうしても「包含ブロック」についての理解が必要となったので急遽追加することにした。
...
もっと言うと、**各要素の生成するボックスは包含ブロックというブロックの中に作られる**ということになる。
...
また、**初期包含ブロックは以下の記述にあるように、ビューポート、つまりウィンドウ(要素が表示される画面)と同じ大きさを持ち、キャンバス(画面)の原点(左上)に固定される。**
...
もう少し噛み砕いていうと、初期包含ブロックは画面全体(ウィンドウ)と同じ大きさを持っている。これは言い換えると、**ウィンドウと同じ大きさを持つ白紙のキャンバス(画用紙でもok)** とも言える。

そして、このウィンドウと同じ大きさを持つ白紙のキャンバスの中に、ルート要素であるhtml要素の生成するボックスが置かれる。

くわえて、先ほどの記述には「初期包含ブロックは、キャンバスの原点に固定される」ともある。

これは白紙の長方形キャンバスがウィンドウの左上に角を合わせてどっこいしょと置かれているイメージを持てばok。
...
プロパティの値が「`relative`」または「`static`」(staticは初期値なので何も設定していない場合はこれ)の場合は、包含ブロックは直近の親要素のコンテンツエッジによって形成される。

そして、次に、包含ブロックの形成されるコンテンツエッジがどこかを調べるために、以下のルールに従って絞り込んでいく。
