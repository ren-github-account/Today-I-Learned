...
結論を書くと、body要素の包含ブロックは**html要素のコンテンツエッジ**に形成されている。

このことを確かめるには、ブラウザのデベロッパーツールを開き、html要素の生成したボックスが見れる画面を確認してみるとよい。

しかし、ここで一つ問題が発生する。

body要素内に何もコードを書いていないのにも関わらず、html要素のコンテンツ領域にheightがあるのだ。
...
body要素のコンテンツ領域の高さが0であることから、body要素の生成するボックス内ではmarginのtopとbottomが重なって相殺が起きている。(だからbody要素のマージンエッジのtopとbottomのうち、大きい方の値がhtml要素のコンテンツ領域の高さとなるんだ!)
