...
結論から書くと、**そのcontent内でブロックフォーマットコンテキストの動きをする。**
...
```
まず、前提として以下の点をおさえていただきたい。

・p要素にはブラウザによってデフォルトで上下に16pxのマージンが付与されているため、何もしない状態だと上下に16pxの間隔をあけながら垂直にレイアウトされていく。

・body要素のdisplayプロパティの値はデフォルトで`block`なことから、body要素の内側ではさきほど書いた「ブロックフォーマットコンテキスト内の動き」に従って要素がレイアウトされていく。
