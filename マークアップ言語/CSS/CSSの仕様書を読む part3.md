...
③`<p class="one">`のマージン「32px」とbody要素のマージン「8px」の**相殺が起き、** 値が大きい方の「32px」が適用される。
***
...
さらに注目すべきは、今度の場合、body要素のデフォルトのマージン`8px`とp.oneのマージン`32px`の**相殺が起きていない点だ。**

マージンの相殺が起きていないとはどういうことかというと、body要素のデフォルトのマージン`8px`に加えて、p.oneのマージン`32px`が適用されていることから、p.oneの生成するボックスの上側は合計で`8+32=40px`空いているということになる。

つまり、一言でいうなら**基準が変わった**ということになる。
...
以上見てきた「新しくブロックフォーマットコンテキストを確立する」やり方は、marginが効かない場合の対処法としても覚えておくと便利だ。
