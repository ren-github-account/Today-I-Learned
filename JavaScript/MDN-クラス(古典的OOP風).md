...
**メソッドはプロトタイプで定義し、データプロパティはコンストラクターで定義する**という考え方を[オブジェクトのプロトタイプ](https://developer.mozilla.org/ja/docs/Learn/JavaScript/Objects/Object_prototypes)(独自プロパティの箇所を参照)のページで学んだが、この考え方はクラスを宣言する時にも使えると思った。
...
プロパティ名の**先頭に`#`を付ける**ことで、そのプロパティは**プライベートデータプロパティ**となりオブジェクトの外から直接アクセスしようとするとエラーになる。
...
**メソッド**に関しても同様にプライベートを付与することができる。以下のように書く。
