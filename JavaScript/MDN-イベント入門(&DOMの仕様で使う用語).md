...
まずDOMという大きなくくりがあって、その中にインターフェイスと呼ばれる仕様があれこれ定められている。DOMはAPIであるため、そのインターフェイスで定められている中に、HTMLのbodyだとかその他の各要素を操作・描画するための色々なAPIが存在する。その操作・描画するための具体的なAPIがDocumentオブジェクトやWindowオブジェクトなどである。
