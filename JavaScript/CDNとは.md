...
* [HTTPリクエスト](#HTTPリクエスト)
* [CORSとは](#CORSとは)
* [同一オリジンポリシー(SOP)](#同一オリジンポリシー\(SOP\))
* [CORSと同一オリジンポリシーの関係](#CORSと同一オリジンポリシーの関係)
* [SOPによって制限されるもの](#SOPによって制限されるもの)
* [cdnjsはcorsの影響を受けない？](#cdnjsはcorsの影響を受けないのか)
...
crossorigin属性は、CORSを設定するための属性。

CORSとは、SOPを緩和するためのプロトコル。

異なるオリジンから画像などを読み込むことを、**クロスオリジン**と呼ぶ。

通常は、Webページの脆弱性を防ぐため、同一オリジンの**リソース**(JavaScriptや画像)のみしかアクセスできないよう制限がかかっている。 (この制限のことをSOPという。)
crossorigin属性を使ってCORSを設定してあげることで、その制限をてきぎ緩めることができる。
...
### cdnjsはcorsの影響を受けないのか

そもそも、**cdnjsとは**、オープンソースコミュニティで運営されているもCDNサービスであり、そこでは多くのJavaScriptライブラリを配信している。

cdnjsからJavaScriptライブラリを読み込むには、`script`タグを使用する。

`script`タグは、昔からあるクロスオリジンアクセス(異なるオリジンから画像などを読み込むこと)の機能であるため、**デフォルトではCORSは使われない。**

一方、`script`タグ内でcrossorigin属性を指定した場合は、CORSが使用されるようになる。

話を戻すと、そもそもCORSが使われないことから、JavaScriptライブラリのChart.jsで、`canvas`を使って描画しても、SOPの制限のところで出てきた「CORSによる許可」がどうたらの話は関わってこない。
