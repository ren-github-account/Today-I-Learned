...
このダブルクォーテーション付きの`"こんにちわ"`が変数`JS`に代入され、`eval`の引数として`const test1 = eval(JS);`の部分で実行されると、変数test1には`eval`の戻り値としてダブルクォーテーションが取れた形の`こんにちわ`が返される。(この時点では戻り値として返されるだけで実行はされない)
