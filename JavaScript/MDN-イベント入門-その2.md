...
イベントバブリングがどういう動きをしたかを復習しておくと、**最も内側にある要素から出発して、その親要素へ、そのまた親要素へ…etc.と内側から外側へと上へ上へと上がっていくのがイベントバブリングだった。**

一方、今回の主題である**イベントキャプチャはその逆で、最も外側の親要素から出発して、その対象となる要素へ向けて深く潜っていくという動きをする。**

実際にイベントとキャプチャを適用するには、`addEventListener()メソッド`の第三引数の`true`と書けば良いのだが、その書き方が以下の2パターン存在する。

パターン2に関しては特に問題はないが、MDNの解説ページで例として挙げられていたコードが`{}`を使用したパターン1の形で戸惑ったのでその意味についてメモしておく。
