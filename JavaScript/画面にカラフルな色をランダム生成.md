...
**注意点**として、以下のコードのように`createElement()メソッド`を使用して要素を生成した後に`idプロパティ`でidを割り当てた場合、`getElementById()メソッド`でアクセスする前に`insertBefore()メソッド`を使用して、要素をHTML文書内にあらかじめ挿入しておく必要がある。
