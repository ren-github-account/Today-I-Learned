...
||意味|
|-|-|
|フレックスコンテナ|フレックスを使用してレイアウトされた文書の領域のこと。|
|フレックスアイテム|フレックスコンテナを作成するには`display:flex`または`display:inline-flex`に設定する必要がある。`display:flex`または`display:inline-flex`に設定された要素の**直下の子要素はフレックスアイテムとなる。**|

**初期値**

`flex-directionプロパティ`の初期値は`row`となる。つまり、デフォルトのフレックスコンテナ内ではアイテムが**横並び**に並んでいく。

`flex-wrapプロパティ`の初期値は`nowrap`となる。つまり、アイテムがコンテナに収まらない場合は、**折り返されずに領域からはみ出る。**
...
上記のCSSコードでわからない書き方が出てきたので以下の表にその意味をまとめた。

||意味|
|-|-|
|`> *`|`>`をは子要素に適用するという意味。`*`は「全てに」という意味。つまり、上記のコードでは、class名`box`を持つ要素の**子要素全てに対して適用する**という意味になる。|
|`border-radius`|どれくらい角を丸めるかを指定できる。英語でradiusは「半径」という意味。1つの数値を設定すると「角丸」となり、2つの数値を設定すると角が「楕円」となる。|
