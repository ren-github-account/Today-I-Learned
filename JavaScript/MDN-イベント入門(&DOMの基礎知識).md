...
`addEventListener()`と一緒によく使われるメソッド。イメージとしては**DOMツリー内をCSSセレクターで検索できる**みたいな感じ。
