...
||意味|
|-|-|
|`requestAnimationFrame()`|`setInterval()`の代わりに使用することができて、こちらの方がアニメーションが滑らかになる。その理由はブラウザで再描画する準備が整った段階で指定した関数(通称:コールバック関数)を実行してくれるから。使い方はコールバック関数(繰り返し実行したい関数)の中で`requestAnimationFrame()`を使って、その繰り返し実行したい関数を呼びだせばok。|
