...
あと` dx = 2; dy = -2;`の部分は無くても動く。理由は、衝突を検出したら跳ね返る処理が動作しているからわざわざ初期化しなくとも問題なく動くのだと考えられる。
...
||意味|
|-|-|
|`requestAnimationFrame()`|`setInterval()`の代わりに使用することができて、こちらの方がアニメーションが滑らかになる。その理由はブラウザで再描画する準備が整った段階で指定した関数(通称:コールバック関数)を実行してくれるから。使い方はコールバック関数(繰り返し実行したい関数)の中で`requestAnimationFrame()`を使って、その繰り返し実行したい関数を呼びだせばok。( 参照:[リファレンス](https://www.webdesignleaves.com/pr/jquery/requestAnimationFrame.html) )|
|コールバック関数|「関数を受け取る関数」に渡すための関数のこと。「関数を受け取る関数」は**高級関数**と呼ばれる。要はコールバック関数は、**関数の引数に渡す関数のこと**とも言い換えられる。なぜコールバック関数のようなことが可能かというと、JavaScriptでは関数は「値」として変数に代入して使うことができることが関係している。( 参照:[古都こと](https://sbfl.net/blog/2019/02/08/javascript-callback-func/) )|
