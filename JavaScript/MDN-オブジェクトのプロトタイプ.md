...
そして、**プロトタイプは、プロトタイプチェーンと呼ばれる連鎖構造を持つ。** 英語でchainは、「同じ種類の一連の項目が列を形成すること」(google翻訳の辞書より)といった意味がある。
...
まず、`Object.assign()メソッド`とはどういうメソッドかというと、**「元となるオブジェクトからプロパティの値を目的のオブジェクトへと代入する」** という機能を持つ。(元となるオブジェクトは複数指定可能) そして、代入が終わると最終的に変更された目的のオブジェクトを戻り値として返す。

**構文**
```
Object.assign(target, source); /* sourceの部分に元となるオブジェクト名を、targetの部分に目的のオブジェクト名を書く
　　　　　　　　　　　　　　　　　　　つまり、右から「元となる部分->目的地」の順番
```
...
このプロトタイプを上手く使いこなせるようになると、複数のオブジェクトに共通の機能を持たせたい時などに重宝しそうだな。
