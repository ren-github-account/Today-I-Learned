...
||意味|
|-|-|
|`HSV`|上記のツール名にも入っている`HSV`は、HとSは基本的に`hsl`と同じだが、`V`だけが違う。この`V`は`Value`の略で「明度」を意味し、**「どれだけ反射しているか」** を表している。上記のコードで指定した`hsl(0 100% 50%)`と同じ数値にして`hsv(0 100% 50%)`として先ほどのカラーピッカーで確認してみると、赤ではなく **「暗めの赤みがかった茶色」** になる。|
