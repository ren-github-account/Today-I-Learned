...
|Node(ノード)|Nodeには大きく2つの意味があって、1つは**DOMとしてのNode**でもう1つは**クラスとしてのNode**になる。**DOMとしてのNodeはノードツリー内のある一点を表す。** **NodeとはブラウザがHTMLを解釈する時に作られるもの。** このNode機能を持つすべてのオブジェクトは、例えばDocumentオブジェクトやElementオブジェクトのクラスから派生したサブクラスがベースとなっている。(つまり、Node機能を持つオブジェクトの親クラスにはDocumentクラスやElementクラスが存在する)|
