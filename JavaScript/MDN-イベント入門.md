...
|DOM| DOMとは、HTMLまたはXMLなどのマークアップ言語の文書を表現・操作するための**API**のこと。**HTMLなどは、一旦ブラウザでノードツリーつまりDOMツリーとして読み込まれ、その読み込まれたそれぞれのノードは文書の一部として表現される。** |
...
|クラスとしてのNode|このNode機能を持つすべてのオブジェクトは、例えばDocumentオブジェクトやElementオブジェクトのクラスから派生したサブクラスがベースとなっている。(つまり、Node機能を持つオブジェクトの親クラスにはDocumentクラスやElementクラスが存在する)|
