...
|EventTargetインターフェース|EventTargetインターフェースは、イベントを受信でき、イベントのリスナーを持つことができるオブジェクトによって実装される。つまり、イベントのターゲットとなるものはすべて、このインターフェースに関連付けられた3つのメソッド(`addEventListener()`,`removeEventListener()`,`dispatchEvent()`)を持つ。**イベントのターゲットとなるものの具体例としては、Elementやその子要素、そしてDocumentオブジェクト と Windowオブジェクトなどがある。** documentオブジェクトとwindowオブジェクトはそれぞれ、DocumentインターフェイスとWindowインターフェイスを持つ。つまりもっと具体的に言えば、**Elementやその子要素、加えてDocumentオブジェクトやWindowオブジェクトなどに対して`addEventListener()`を使用することができる**ってことだな。|
