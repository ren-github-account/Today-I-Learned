...
|オブジェクト|「HTML DOM」関係のMDNの解説ページに、よく**オブジェクト**という言葉が登場する。この文脈におけるオブジェクトは、**「要素やテキスト」のこと**を表している。ノードとの関係性でいうと、この**オブジェクト(要素やテキスト)をその内部に記録している**のが**ノード**ということになる。[参照:HTML文書の構造](https://developer.mozilla.org/ja/docs/Web/API/HTML_DOM_API)。もっといえば、MDNのページに　**「DOMは文書をノードとオブジェクトで表現します。」** ( 参照:[DOM の紹介](https://developer.mozilla.org/ja/docs/Web/API/Document_Object_Model/Introduction) )とあるように、HTMLの要素達がブラウザで解釈される時DOMツリーに変換される。その時に各要素はDOMで定義された**DocumentオブジェクトやWindowオブジェクトのAPI**を利用することで、**各要素を操作することが可能となる。**(参照:DOMの紹介)|
