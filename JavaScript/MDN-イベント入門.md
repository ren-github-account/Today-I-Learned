...
|HTML DOM|まずDOMという大きなくくりがあって、さらにその中に**HTMLを操作・描画するための「HTML DOM」が存在する。** HTMLが書かれた文書は、DOMの中の**Documentインターフェイス**を使用して表現される。より具体的には、**Documentインターフェイスのインスタンス(具体例:bodyなどがある)** で表現される。HTML DOMで定義されているインターフェイスとその継承関係について詳しくは[こちら](https://developer.mozilla.org/ja/docs/Web/API/HTML_DOM_API)、DOMとHTML DOMの関係については[こちら](https://developer.mozilla.org/ja/docs/Web/API/Document_Object_Model#html_dom)。|
